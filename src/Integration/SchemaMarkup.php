<?php
/**
 * Schema.org markup integration for ReviewApp.
 *
 * @package ReviewApp
 */
...
class SchemaMarkup {
	...
	public function generate_product_schema( $product_id ) {
		...
		$avg_stars = get_post_meta( $product_id, '_reviewapp_avg_stars', true );
		$review_count = get_post_meta( $product_id, '_reviewapp_reviews_count', true );
		...
		// Fetch individual reviews from ReviewApp API.
	...
	/**
	 * Fetch approved reviews from ReviewApp API for schema markup.
	 *
	 * @param int $product_id WooCommerce product ID.
	...
	private function fetch_reviews_for_schema( $product_id ) {
		...
		$cache_key = 'reviewapp_schema_reviews_' . $product_id;
		...
		$response = wp_remote_get(
			reviewapp_get_api_url() . "/api/stores/{$store_id}/products/{$product_id}/reviews",
			array(
				'timeout'   => 10,
	...
	public function output_product_schema( $product_id ) {
		...
		$enabled = get_option( 'reviewapp_enable_schema', 'yes' );
		if ( 'yes' !== $enabled ) {
			return;
		}
		...
		$schema = apply_filters( 'reviewapp_product_schema', $schema, $product_id );
