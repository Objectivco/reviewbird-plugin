# reviewbird Schema Markup Feature

## Overview

The reviewbird WordPress plugin automatically adds Google-compliant structured data (JSON-LD schema) to WooCommerce product pages. This enables rich snippets in search results, displaying star ratings, review counts, and other product information directly in Google search results.
...
- WordPress 5.0+
- WooCommerce 5.0+
- reviewbird plugin connected to a reviewbird store
- Products must have reviews in reviewbird
...
1. Go to WordPress Admin > Settings > reviewbird (`/wp-admin/options-general.php?page=reviewbird-settings`)
...
```php
add_filter( 'reviewbird_enable_schema', '__return_false' );
```
...
You can modify the schema data before output using the `reviewbird_product_schema` filter:

```php
add_filter( 'reviewbird_product_schema', function( $schema, $product_id ) {
...
```
reviewbird_schema_reviews_{product_id}
```
...
1. Product ratings are updated via the reviewbird webhook
...
```php
$schema_markup = new \reviewbird\Integration\SchemaMarkup();
...
```php
global $wpdb;
$wpdb->query( "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_reviewbird_schema_reviews_%'" );
$wpdb->query( "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_timeout_reviewbird_schema_reviews_%'" );
```
...
3. **Check option**: Verify `reviewbird_enable_schema` option is set to 'yes'
4. **Theme compatibility**: Some themes may remove `wp_head` hook

### No Reviews in Schema

1. **Check API connection**: Ensure reviewbird store is connected
2. **Verify reviews exist**: Product must have approved reviews in reviewbird
...
do_action( 'reviewbird_before_schema_output', $product_id );

// After schema is output
do_action( 'reviewbird_after_schema_output', $product_id );
```
...
apply_filters( 'reviewbird_product_schema', $schema, $product_id );

// Modify cache duration (in seconds, default is 4 hours)
apply_filters( 'reviewbird_schema_cache_duration', 4 * HOUR_IN_SECONDS );

// Modify number of reviews to include
apply_filters( 'reviewbird_schema_review_limit', 10 );
```
...
3. Contact reviewbird support for API-related issues
