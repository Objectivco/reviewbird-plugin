...
export default function ConnectionPanel({ settings, onSave, saving }) {
	const handleOAuthConnect = () => {
		...
		form.method = 'POST';
		form.action = window.reviewbirdAdmin?.ajaxUrl || '/wp-admin/admin-ajax.php';
		
		const actionField = document.createElement('input');
		actionField.type = 'hidden';
		actionField.name = 'action';
		actionField.value = 'reviewbird_start_oauth';
	...
	const handleDisconnect = async () => {
		if (confirm(__('Are you sure you want to disconnect from reviewbird?', 'reviewbird-reviews'))) {
			try {
		...
		<div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
			<div className="px-6 py-5 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
				<h2 className="text-xl font-semibold text-gray-900">
					{__('Connection', 'reviewbird-reviews')}
				</h2>
				<StatusIndicator
				...
				{!isConnected ? (
					<div className="text-center py-8">
						...
						<h3 className="text-lg font-medium text-gray-900 mb-2">
							{__('Connect to reviewbird', 'reviewbird-reviews')}
						</h3>
						<p className="text-gray-600 mb-6 max-w-md mx-auto">
							{__('Start collecting and displaying reviews by connecting your store to reviewbird', 'reviewbird-reviews')}
						</p>
						<Button
							variant="primary"
						...
						>
							{__('Connect Now', 'reviewbird-reviews')}
						</Button>
					</div>
				) : (
					<div className="space-y-4">
							...
							<div className="flex items-start">
								...
								<div className="ml-3">
									<h3 className="text-sm font-medium text-green-800">
										{__('Your store is connected', 'reviewbird-reviews')}
									</h3>
									{settings.store_id && (
										<p className="text-sm text-green-700 mt-1">
											{__('Store ID:', 'reviewbird-reviews')} <code className="font-mono">{settings.store_id}</code>
										</p>
									)}
								</div>
							</div>
						...
						>
							{__('Disconnect', 'reviewbird-reviews')}
						</button>
					</div>
				)}
			</div>
