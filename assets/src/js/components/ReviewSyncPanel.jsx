...
export default function ReviewSyncPanel({ isConnected, productsAreSynced }) {
	...
	const loadSyncStatus = async () => {
		try {
			const data = await apiFetch({ path: '/reviewbird/v1/sync/reviews/status' });
			setSyncStatus(data);
			setError(null);
		} catch (err) {
			console.error('Failed to load review sync status:', err);
			setError(err.message || __('Failed to load review sync status', 'reviewbird-reviews'));
		} finally {
			setLoading(false);
		}
	};
	...
	const startSync = async () => {
		...
		try {
			await apiFetch({
				path: '/reviewbird/v1/sync/reviews/start',
				method: 'POST',
			});
			// Reload status immediately
			await loadSyncStatus();
		} catch (err) {
			setError(err.message || __('Failed to start review sync', 'reviewbird-reviews'));
		} finally {
			setSyncing(false);
		}
	};
		...
		<div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
			<div className="px-6 py-5 border-b border-gray-200 bg-gray-50">
				<h2 className="text-xl font-semibold text-gray-900">
					{__('Review Sync', 'reviewbird-reviews')}
				</h2>
			</div>
						...
						<div className="flex">
							...
							<div className="ml-3">
								<h3 className="text-sm font-medium text-yellow-800">
									{__('Products Must Be Synced First', 'reviewbird-reviews')}
								</h3>
								<p className="text-sm text-yellow-700 mt-1">
									{__('Please sync your products before syncing reviews. Reviews are linked to products.', 'reviewbird-reviews')}
								</p>
							</div>
						</div>
					</div>
					...
					<div className="space-y-4">
						<div className="flex items-center justify-between text-sm">
							<span className="font-medium text-gray-700">
								{__('Syncing reviews...', 'reviewbird-reviews')}
							</span>
							...
							<div className="flex">
								...
								<div className="ml-3">
									<p className="text-sm text-blue-800">
										{__('Sync in progress. This may take a few minutes...', 'reviewbird-reviews')}
									</p>
								</div>
							</div>
						</div>
					...
					<div className="space-y-4">
						<div className="grid grid-cols-3 gap-4">
							<div className="bg-gray-50 rounded-lg p-4">
								...
								<div className="text-sm text-gray-600">
									{__('Approved reviews', 'reviewbird-reviews')}
								</div>
							</div>
							...
							<>
								<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
									<div className="flex">
										...
										<div className="ml-3">
											<h3 className="text-sm font-medium text-yellow-800">
												{__('Sync Required', 'reviewbird-reviews')}
											</h3>
											<p className="text-sm text-yellow-700 mt-1">
												{__('You have approved reviews that need to be synced to reviewbird.', 'reviewbird-reviews')}
											</p>
										</div>
									</div>
								</div>
								...
								>
									{syncing ? __('Starting sync...', 'reviewbird-reviews') : __('Sync Reviews', 'reviewbird-reviews')}
								</button>
							</>
								...
								<div className="flex">
									...
									<div className="ml-3">
										<p className="text-sm text-green-800">
											{__('All reviews are synced', 'reviewbird-reviews')}
										</p>
									</div>
								</div>
							</div>
						...
						{!productsAreSynced && (
							<p className="text-xs text-gray-500">
								{__('Sync products first to enable review sync', 'reviewbird-reviews')}
							</p>
						)}
					</div>
				)}
