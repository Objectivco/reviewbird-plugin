...
export default function SchemaPanel({ settings, onSave, saving, isConnected }) {
	...
	const handleToggle = async () => {
		...
		try {
			...
			formData.append('action', 'reviewbird_update_schema_setting');
			...
			alert(__('Failed to update schema setting. Please try again.', 'reviewbird-reviews'));
		} finally {
			setIsSaving(false);
		}
	};
			...
			<div className="flex items-start justify-between">
				<div className="flex-1">
					<h2 className="text-lg font-semibold text-gray-900">
						{__('SEO Schema Markup', 'reviewbird-reviews')}
					</h2>
					<p className="mt-1 text-sm text-gray-600">
						{__('Enable Google-compliant structured data (JSON-LD schema) on product pages for rich snippets in search results.', 'reviewbird-reviews')}
					</p>
					...
					<div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
						<div className="flex">
							...
							<div className="ml-3 text-sm text-blue-800">
								<p className="font-medium">{__('What is Schema Markup?', 'reviewbird-reviews')}</p>
								<ul className="mt-2 space-y-1">
									<li>• {__('Displays star ratings in Google search results', 'reviewbird-reviews')}</li>
									<li>• {__('Shows review counts and product information', 'reviewbird-reviews')}</li>
									<li>• {__('Can increase click-through rates by up to 30%', 'reviewbird-reviews')}</li>
									<li>• {__('Reviews are cached for 4 hours for optimal performance', 'reviewbird-reviews')}</li>
								</ul>
							</div>
						</div>
					</div>
				...
				<div className="ml-6">
					...
					<p className="mt-2 text-xs text-gray-500 text-right" style={{ margin: '8px 0 0 0', fontSize: '12px', lineHeight: '1.4' }}>
						{enableSchema ? __('Enabled', 'reviewbird-reviews') : __('Disabled', 'reviewbird-reviews')}
					</p>
				</div>
			</div>
			...
			{enableSchema && (
				<div className="mt-4 pt-4 border-t border-gray-200">
					<p className="text-sm text-gray-600">
						{__('Schema markup is active on all WooCommerce product pages.', 'reviewbird-reviews')}
					</p>
					<div className="mt-3 space-y-2">
						...
						>
							{__('Test with Google Rich Results', 'reviewbird-reviews')}
						...
						<a
							...
							className="inline-flex items-center text-sm text-indigo-600 hover:text-indigo-700"
						>
							{__('Validate Schema', 'reviewbird-reviews')}
							<svg className="ml-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
							</svg>
						</a>
